...
The previous section showed that the CTS can exist in a homogeneous scalar regime where
$$
\Phi(\mathbf{x},t) = \Phi_0
$$
...
\nabla \Phi = 0
$$
and no spatial directions are distinguished.
...
$$
added to the homogeneous state.
...
\Phi = \Phi_0 + \delta\Phi.
$$
Expanding to first order in ( $\delta\Phi) gives$
$$
\partial_t (\delta\Phi)
r,\delta\Phi
...
a\nabla^2(\delta\Phi)
u\nabla^4(\delta\Phi)
3s\Phi_0^2,\delta\Phi.
$$
Thus the linearized equation becomes
$$
\partial_t (\delta\Phi)
-(r+3s\Phi_0^2)\delta\Phi
+
a\nabla^2(\delta\Phi)
u\nabla^4(\delta\Phi).
$$
...
For perturbations to grow we require
$$
\omega(k) > 0.
$$
...
$$
If this condition is satisfied for some value of
$$
k,
$$
then the symmetric state becomes unstable.
...
The fastest-growing mode corresponds to the value of
$$
k
$$
...
k^2 = -\frac{a}{2u}.
$$
Thus the characteristic wavelength becomes
$$
\lambda_c = \frac{2\pi}{k}.
$$
...
\alpha_1 ||\nabla\Phi||^2.
$$
If gradient retention grows large enough relative to loss mechanisms, the configuration may satisfy
$$
S \ge 1.
$$
...
The CTS as a Persistence-Bearing Field
