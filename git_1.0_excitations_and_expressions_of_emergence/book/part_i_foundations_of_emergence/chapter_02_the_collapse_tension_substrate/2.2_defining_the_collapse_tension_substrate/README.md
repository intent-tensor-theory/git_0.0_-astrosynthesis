...
Let the substrate be described by a scalar field $\Phi(\mathbf{x}, t)$.
This field represents the local structural potential of the substrate.
The field is defined over a continuous domain, which for now may be treated as a three-dimensional coordinate space $\mathbf{x} \in \mathbb{R}^3$.
Thus
...
To determine the dynamics of the field we introduce an energy functional $E[\Phi]$.
...
The linear term $-r\Phi$ represents structural collapse.
If $r > 0$, then fluctuations tend to decay.
This term drives the system toward $\Phi = 0$.
...
Stationary configurations satisfy $\partial_t \Phi = 0$.
Thus
...
The sign of $\omega$ determines stability.
If $\omega > 0$ the mode grows.
If $\omega < 0$ the mode decays.

## 2.2.10 Mode selection

The growth rate of a mode depends on its wavenumber $k = |\mathbf{k}|$.
The dominant mode corresponds to the value of $k$ that maximizes $\omega(k)$.
