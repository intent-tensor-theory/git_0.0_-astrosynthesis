...
Consider a localized closed structure with center $\mathbf{x}_0$.
...
Each channel carries structural flux $F_i$.
...
In the symmetric case each channel carries equal structural flux $F_i = F$.
...
Let $\delta r(\theta,\phi)$ represent radial deformation of the shell.
...
where $k$ is restoring stiffness from fan locking and $\gamma$ is the deformation growth rate.
